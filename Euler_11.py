...
prod = list()
for i in range(17): # ---------------------------------------------- diagonally forward
	for j in range(17):
		prod.append(grid[i][j] * grid[i+1][j+1] * grid[i+2][j+2] * grid[i+3][j+3])
for i in range(17): # --------------------------------------------- diagonally backward
	for j in range(3, 20):
		prod.append(grid[i][j] * grid[i+1][j-1] * grid[i+2][j-2] * grid[i+3][j-3])
...
print("The greatest product of four adjacent numbers in the same direction = %d" %max(prod))